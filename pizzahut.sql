...
select round(avg(quantity),0) as average_number
from
...
(select round(sum(order_details.quantity * pizzas.price),0) as total_sales
from order_details join pizzas
...
join order_details
on order_details.pizza_id = pizzas.pizza_id
group by pizza_types.category, pizza_types.name order by revenue) as a)as b
where Ranking <=3;